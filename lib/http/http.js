...
import TokenStorage from '../cache/token-storage'
...
//  config.headers['X-Auth-Token'] =  'eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiIxODM5NjU1NTU3NSIsInN1YiI6IjE4Mzk2NTU1NTc1IiwiaWF0IjoxNTM2MDY4MDM2fQ.GJAuUykhHxQKGa2OER8AsxkeDfXcoN8Ek-JR38BoYKg'
    if(TokenStorage.get()){
    	config.headers['X-Auth-Token'] = TokenStorage.get()
    }
    Http.combine(config)
...
//      config.headers['Authorization'] = AuthEncrypt.getMac(config.method, config.baseURL + decodeURIComponent(config.url), token)
    ...
    var completeCallback = function (response) {
    	if(response&&response.respCode == -1){
    		showError(response.message)
    	} else if(response&&response.respCode == 403){
    		TokenStorage.clear()
    		global.login && global.login()
    	} else if(response&&response.respCode == 500){
    		global.sysLocked && global.sysLocked(response)
    	}
